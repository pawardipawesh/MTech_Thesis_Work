...
public class AssimulatePrediction_FileToBeClassified {
	...
	public static void Assimulate(String f1,String f2,String f3)throws Exception{
		...
		while((lP=predictionFile.readLine())!=null){
			...
			for(int i=4;i<wordsArrayForFileToBeClassified.length-4;i++){
				G=G.concat(wordsArrayForFileToBeClassified[i]+" ");
				
			}
			prediction_FileToBeClassified.write(wordsArrayForFileToBeClassified[0]+"\t"+wordsArrayForFileToBeClassified[1]+"\t"+wordsArrayForFileToBeClassified[2]+"\t"+String.valueOf(sn)+"\t"+G+"\t"+s+"\t"+wordsArrayForpredictionFile[wordsArrayForpredictionFile.length-1]+"\t##\n");
