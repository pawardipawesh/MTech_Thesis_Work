...
i=$(echo "$i-1" | bc -l)
 #------------------------------Obtain prediction For Final THW -----------------------------------------------------------------------
		java -classpath $path/weka.jar:$path/libsvm.jar weka.classifiers.meta.FilteredClassifier -T $path/TestSet/TestSet.arff  -l $path/Models/SVMx10_iteration$i.model -p 0  > $path/Predictions/PredictionForFinalTHW.txt
